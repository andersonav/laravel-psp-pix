...
namespace Alves\Pix\Concerns;

use Alves\Pix\Exceptions\InvalidMerchantInformationException;
use Alves\Pix\Exceptions\InvalidTransactionIdException;
use Alves\Pix\Pix;
...
trait InteractsWithPayload
{
    ...
    {
        $gui = $this->formatValue(Pix::MERCHANT_ACCOUNT_INFORMATION_GUI, config('laravel-psp-pix.gui', 'br.gov.bcb.pix'));
    ...
    {
        return $this->formatValue(Pix::TRANSACTION_CURRENCY, config('laravel-psp-pix.transaction_currency_code', '986'));
    }

    protected function getCountryCode(): string
    {
        return $this->formatValue(Pix::COUNTRY_CODE, config('laravel-psp-pix.country_code', 'BR'));
    }

    /**
     * @throws \Alves\Pix\Exceptions\PixException
     */
